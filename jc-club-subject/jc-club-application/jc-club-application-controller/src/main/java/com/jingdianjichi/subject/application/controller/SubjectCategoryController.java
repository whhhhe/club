...
public class SubjectCategoryController {
    ...
    public Result<List<SubjectCategoryDTO>> queryPrimaryCategory() {
        try {
            ...
            List<SubjectCategoryDTO> dtoList = new LinkedList<>();
            dtoList = SubjectCategoryDTOConverter.INSTANCE.convertBoToCategoryDTOList(subjectCategoryBOList);
            return Result.ok(dtoList);
        } catch (Exception e) {
            log.error("SubjectCategoryController.queryPrimaryCategory.error:{}", e.getMessage(), e);
            return Result.fail("查询失败");
     ...
     * 查询对应二级分类
     * */
    @GetMapping("/querySecondCategory")
    public Result<List<SubjectCategoryDTO>> querySecondCategory(@RequestParam("parentId") int parentId) {
        try {
            Preconditions.checkNotNull(parentId, "parentId不能为空");
            List<SubjectCategoryBO> subjectCategoryBOList = subjectCategoryDomainService.querySecondCategory(parentId);
            List<SubjectCategoryDTO> dtoList = new LinkedList<>();
            dtoList = SubjectCategoryDTOConverter.INSTANCE.convertBoToCategoryDTOList(subjectCategoryBOList);
            return Result.ok(dtoList);
        } catch (Exception e) {
