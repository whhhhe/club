package com.jingdianjichi.interview.server.util;
...
public class HttpUtils {
     ...
     * post 请求
     */
    public static String executePost(String url, String reqArgs, Map<String, String> headerMap) {
        String result = null;
        HttpPost httpPost = new HttpPost(url);

        RequestConfig config = RequestConfig.custom()
                .setConnectTimeout(Timeout.ofSeconds(30))         // 连接超时
                .setResponseTimeout(Timeout.ofSeconds(60))        // 响应读取超时
                .setConnectionRequestTimeout(Timeout.ofSeconds(10)) // 获取连接超时
                .build();
        ...
        try (CloseableHttpResponse response = getHttpclient().execute(httpPost)) {
            if (response.getCode() == HttpStatus.SC_OK || response.getCode() == HttpStatus.SC_CREATED) {
                ...
                log.error("请求失败，HTTP状态码：{}", response.getCode());
            }
        } catch (SocketTimeoutException e) {
            log.error("❌ 请求超时！{}", e.getMessage(), e);
        } catch (Exception e) {
            log.error("❌ POST 请求异常！{}", e.getMessage(), e);
        }
        return result;
    }
