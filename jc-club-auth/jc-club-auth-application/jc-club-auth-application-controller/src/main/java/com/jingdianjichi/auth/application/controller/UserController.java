...
public class UserController {
    ...
    public Result<Long> getRoleIdByUserId(@RequestParam("userId") String userId){
        try {
            Preconditions.checkArgument(!StringUtils.isBlank(userId), "用户名不能为空!");
            return Result.ok(authUserDomainService.getRoleIdByUserId(userId));
        } catch (Exception e) {
            log.error("UserController.doLogin.error:{}", e.getMessage(), e);
    ...
    public Result<Map<Long, String>> queryUserNames(List<String> list){
        try{
            if(log.isInfoEnabled()){
                log.info("UserController.queryUserNames.list:{}", list);
            }
            Map<String, String> res = new HashMap<>();
            if(list != null && list.size() > 0){
                List<String> collect = authUserDomainService.listUserInfoByIds(list).stream().map(AuthUserBO::getNickName).collect(Collectors.toList());

                for(int i = 0; i < list.size(); i++){
                    res.put(list.get(i), collect.get(i));
                }
            }
            return Result.ok(res);
        }catch (Exception e){
            log.error("UserController.queryUserNames.error:{}", e.getMessage());
            return Result.fail("根据names查询nicknames失败");
        }
    }
}
